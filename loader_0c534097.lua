...
local CONFIG = {
    ...
    BG = Color3.fromRGB(15, 25, 22)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_slWRbCcXJgAS") then CoreGui["KeySystem_slWRbCcXJgAS"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_slWRbCcXJgAS"
sg.Parent = CoreGui

local EOpWvUeeXU = Instance.new("Frame")
EOpWvUeeXU.Name = "AsqdYBQXoOmk"
EOpWvUeeXU.Parent = sg
EOpWvUeeXU.BackgroundColor3 = CONFIG.BG
EOpWvUeeXU.Position = UDim2.new(0.5, -144, 0.5, -85)
EOpWvUeeXU.Size = UDim2.new(0, 289, 0, 170)
EOpWvUeeXU.ClipsDescendants = true

    local grad = Instance.new("UIGradient")
    grad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, EOpWvUeeXU.BackgroundColor3),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(45, 50, 57))
    })
    grad.Rotation = 180
    grad.Parent = EOpWvUeeXU
    

local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 13)
UICorner.Parent = EOpWvUeeXU

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = EOpWvUeeXU
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 1.6
UIStroke.Transparency = 0.4

local title = Instance.new("TextLabel")
title.Parent = EOpWvUeeXU
...
-- Smooth Dragging
local d, i, s, p
EOpWvUeeXU.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = EOpWvUeeXU.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
EOpWvUeeXU.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        EOpWvUeeXU.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(EOpWvUeeXU, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -144, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
