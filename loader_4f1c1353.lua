...
local CONFIG = {
    ...
    BG = Color3.fromRGB(26, 29, 33)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_tZfhzzkyOLhv") then CoreGui["KeySystem_tZfhzzkyOLhv"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_tZfhzzkyOLhv"
sg.Parent = CoreGui

local gctFGophp = Instance.new("Frame")
gctFGophp.Name = "iGQcPBRIZ"
gctFGophp.Parent = sg
gctFGophp.BackgroundColor3 = CONFIG.BG
gctFGophp.Position = UDim2.new(0.5, -141, 0.5, -87)
gctFGophp.Size = UDim2.new(0, 283, 0, 175)
gctFGophp.ClipsDescendants = true


local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 10)
UICorner.Parent = gctFGophp

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = gctFGophp
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 1.4
UIStroke.Transparency = 0.6

local title = Instance.new("TextLabel")
title.Parent = gctFGophp
...
-- Smooth Dragging
local d, i, s, p
gctFGophp.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = gctFGophp.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
gctFGophp.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        gctFGophp.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(gctFGophp, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -141, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
