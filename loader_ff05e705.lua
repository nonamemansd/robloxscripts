...
local CONFIG = {
    ...
    BG = Color3.fromRGB(30, 21, 33)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_nUxzPtdWzEn") then CoreGui["KeySystem_nUxzPtdWzEn"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_nUxzPtdWzEn"
sg.Parent = CoreGui

local eDsMClw = Instance.new("Frame")
eDsMClw.Name = "vSaNmxUfoFvg"
eDsMClw.Parent = sg
eDsMClw.BackgroundColor3 = CONFIG.BG
eDsMClw.Position = UDim2.new(0.5, -155, 0.5, -85)
eDsMClw.Size = UDim2.new(0, 310, 0, 170)
eDsMClw.ClipsDescendants = true

    local grad = Instance.new("UIGradient")
    grad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, eDsMClw.BackgroundColor3),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(36, 51, 56))
    })
    grad.Rotation = 90
    grad.Parent = eDsMClw
...
-- Smooth Dragging
local d, i, s, p
eDsMClw.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = eDsMClw.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
eDsMClw.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        eDsMClw.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(eDsMClw, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -155, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
