...
local CONFIG = {
    ...
    BG = Color3.fromRGB(15, 27, 31)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_BXZRIyH") then CoreGui["KeySystem_BXZRIyH"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_BXZRIyH"
sg.Parent = CoreGui

local JAozKoUVR = Instance.new("Frame")
JAozKoUVR.Name = "rciHdmex"
JAozKoUVR.Parent = sg
JAozKoUVR.BackgroundColor3 = CONFIG.BG
JAozKoUVR.Position = UDim2.new(0.5, -148, 0.5, -95)
JAozKoUVR.Size = UDim2.new(0, 296, 0, 190)
JAozKoUVR.ClipsDescendants = true


local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 9)
UICorner.Parent = JAozKoUVR

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = JAozKoUVR
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 1.7
UIStroke.Transparency = 0.5

local title = Instance.new("TextLabel")
title.Parent = JAozKoUVR
...
-- Smooth Dragging
local d, i, s, p
JAozKoUVR.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = JAozKoUVR.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
JAozKoUVR.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        JAozKoUVR.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(JAozKoUVR, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -148, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
