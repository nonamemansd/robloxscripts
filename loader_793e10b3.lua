--[[ 
    LOADER ID: e5d79e1b97
    BUILD TYPE: CLOUD
]]
local CONFIG = {
    Title = "Aether External",
    ...
    BG = Color3.fromRGB(17, 27, 35)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_LPUQmIKqWk") then CoreGui["KeySystem_LPUQmIKqWk"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_LPUQmIKqWk"
sg.Parent = CoreGui

local SCkXLEJjIU = Instance.new("Frame")
SCkXLEJjIU.Name = "sTlxFmzCRPAL"
SCkXLEJjIU.Parent = sg
SCkXLEJjIU.BackgroundColor3 = CONFIG.BG
SCkXLEJjIU.Position = UDim2.new(0.5, -147, 0.5, -89)
SCkXLEJjIU.Size = UDim2.new(0, 294, 0, 179)
SCkXLEJjIU.ClipsDescendants = true

    local grad = Instance.new("UIGradient")
    grad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, SCkXLEJjIU.BackgroundColor3),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(36, 50, 43))
    })
    grad.Rotation = 180
    grad.Parent = SCkXLEJjIU
    

local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 14)
UICorner.Parent = SCkXLEJjIU

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = SCkXLEJjIU
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 1.5
UIStroke.Transparency = 0.6

local title = Instance.new("TextLabel")
title.Parent = SCkXLEJjIU
...
KeyInput.Parent = SCkXLEJjIU
KeyInput.BackgroundColor3 = Color3.fromRGB(30, 30, 35)
KeyInput.Position = UDim2.new(0.1, 0, 0.32, 0)
KeyInput.Size = UDim2.new(0.8, 0, 0, 35)
KeyInput.Font = Enum.Font.Gotham
KeyInput.PlaceholderText = "Paste license..."
KeyInput.Text = ""
...
-- Smooth Dragging
local d, i, s, p
SCkXLEJjIU.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = SCkXLEJjIU.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
SCkXLEJjIU.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        SCkXLEJjIU.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(SCkXLEJjIU, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -147, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
