...
local CONFIG = {
    ...
    BG = Color3.fromRGB(28, 21, 30)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_pDEhDGIsxFl") then CoreGui["KeySystem_pDEhDGIsxFl"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_pDEhDGIsxFl"
sg.Parent = CoreGui

local JKPyzJBJ = Instance.new("Frame")
JKPyzJBJ.Name = "EvLXKYLz"
JKPyzJBJ.Parent = sg
JKPyzJBJ.BackgroundColor3 = CONFIG.BG
JKPyzJBJ.Position = UDim2.new(0.5, -140, 0.5, -89)
JKPyzJBJ.Size = UDim2.new(0, 280, 0, 179)
JKPyzJBJ.ClipsDescendants = true

    local grad = Instance.new("UIGradient")
    grad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, JKPyzJBJ.BackgroundColor3),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(42, 49, 48))
    })
    grad.Rotation = 90
    grad.Parent = JKPyzJBJ
    

local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 6)
UICorner.Parent = JKPyzJBJ

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = JKPyzJBJ
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 1.6
UIStroke.Transparency = 0.6

local title = Instance.new("TextLabel")
title.Parent = JKPyzJBJ
...
-- Smooth Dragging
local d, i, s, p
JKPyzJBJ.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = JKPyzJBJ.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
JKPyzJBJ.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        JKPyzJBJ.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(JKPyzJBJ, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -140, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
