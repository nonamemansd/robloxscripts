...
local CONFIG = {
    ...
    BG = Color3.fromRGB(16, 26, 32)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_GGaGOTAHCmEx") then CoreGui["KeySystem_GGaGOTAHCmEx"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_GGaGOTAHCmEx"
sg.Parent = CoreGui

local lftqJTXwtLZ = Instance.new("Frame")
lftqJTXwtLZ.Name = "JQrHeCleu"
lftqJTXwtLZ.Parent = sg
lftqJTXwtLZ.BackgroundColor3 = CONFIG.BG
lftqJTXwtLZ.Position = UDim2.new(0.5, -142, 0.5, -88)
lftqJTXwtLZ.Size = UDim2.new(0, 284, 0, 177)
lftqJTXwtLZ.ClipsDescendants = true


local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 13)
UICorner.Parent = lftqJTXwtLZ

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = lftqJTXwtLZ
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 1.6
UIStroke.Transparency = 0.5

local title = Instance.new("TextLabel")
title.Parent = lftqJTXwtLZ
...
-- Smooth Dragging
local d, i, s, p
lftqJTXwtLZ.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = lftqJTXwtLZ.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
lftqJTXwtLZ.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        lftqJTXwtLZ.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(lftqJTXwtLZ, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -142, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
