...
local CONFIG = {
    ...
    BG = Color3.fromRGB(30, 19, 23)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_qrdtFjXYIq") then CoreGui["KeySystem_qrdtFjXYIq"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_qrdtFjXYIq"
sg.Parent = CoreGui

local NipumUhYlAv = Instance.new("Frame")
NipumUhYlAv.Name = "SXIqHEBbBcy"
NipumUhYlAv.Parent = sg
NipumUhYlAv.BackgroundColor3 = CONFIG.BG
NipumUhYlAv.Position = UDim2.new(0.5, -140, 0.5, -92)
NipumUhYlAv.Size = UDim2.new(0, 280, 0, 184)
NipumUhYlAv.ClipsDescendants = true

    local grad = Instance.new("UIGradient")
    grad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, NipumUhYlAv.BackgroundColor3),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(45, 36, 50))
    })
    grad.Rotation = 90
    grad.Parent = NipumUhYlAv
    

local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 7)
UICorner.Parent = NipumUhYlAv

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = NipumUhYlAv
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 1.9
UIStroke.Transparency = 0.6

local title = Instance.new("TextLabel")
title.Parent = NipumUhYlAv
...
-- Smooth Dragging
local d, i, s, p
NipumUhYlAv.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = NipumUhYlAv.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
NipumUhYlAv.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        NipumUhYlAv.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(NipumUhYlAv, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -140, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
