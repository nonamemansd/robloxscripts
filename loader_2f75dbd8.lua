...
local CONFIG = {
    ...
    BG = Color3.fromRGB(22, 24, 23)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_uKMzBvTlk") then CoreGui["KeySystem_uKMzBvTlk"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_uKMzBvTlk"
sg.Parent = CoreGui

local zFCzbnmztAvr = Instance.new("Frame")
zFCzbnmztAvr.Name = "ulRzoKQBjit"
zFCzbnmztAvr.Parent = sg
zFCzbnmztAvr.BackgroundColor3 = CONFIG.BG
zFCzbnmztAvr.Position = UDim2.new(0.5, -142, 0.5, -87)
zFCzbnmztAvr.Size = UDim2.new(0, 285, 0, 175)
zFCzbnmztAvr.ClipsDescendants = true


local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 7)
UICorner.Parent = zFCzbnmztAvr

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = zFCzbnmztAvr
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 1.2
UIStroke.Transparency = 0.5

local title = Instance.new("TextLabel")
title.Parent = zFCzbnmztAvr
...
-- Smooth Dragging
local d, i, s, p
zFCzbnmztAvr.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = zFCzbnmztAvr.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
zFCzbnmztAvr.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        zFCzbnmztAvr.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(zFCzbnmztAvr, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -142, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
