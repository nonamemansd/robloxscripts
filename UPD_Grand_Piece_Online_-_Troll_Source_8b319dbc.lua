...
task.delay(1, function()
	if Library.ToggleKeybind ~= Enum.KeyCode.RightShift then
		Library.ToggleKeybind = Enum.KeyCode.RightShift
	end

	Library:Notify("You have to press Right Shift to hide/unhide the script.", 10)
end)
...
AimGroup:AddToggle("AimEnabled", {Text = "Enable Aim", Default = false})
AimGroup:AddToggle("ShowFOV", {Text = "Show FOV", Default = true})
...
AimGroup:AddButton({
	...
	Func = function()
		if setclipboard then
			...
			Library:Notify("Invite copied to clipboard!", 3)
		end
	end,
})
...
VisualGroup:AddToggle("ESPEnabled", {Text = "Enable ESP", Default = false})
...
MoveGroup:AddToggle("WallClimbEnabled", {Text = "Enable Wall Climb", Default = false})
...
local function touchingWall()
	...
	return Workspace:Raycast(hrp.Position, hrp.CFrame.LookVector * 2.5, params) ~= nil
end
...
TrackerGroup:AddToggle("GunTrackerEnabled", {Text = "Enable Tracker", Default = false})
