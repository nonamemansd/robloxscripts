...
local CONFIG = {
    ...
    BG = Color3.fromRGB(20, 15, 21)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_zizztCrGE") then CoreGui["KeySystem_zizztCrGE"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_zizztCrGE"
sg.Parent = CoreGui

local TemLvzJUKXr = Instance.new("Frame")
TemLvzJUKXr.Name = "wGJnmKik"
TemLvzJUKXr.Parent = sg
TemLvzJUKXr.BackgroundColor3 = CONFIG.BG
TemLvzJUKXr.Position = UDim2.new(0.5, -155, 0.5, -98)
TemLvzJUKXr.Size = UDim2.new(0, 310, 0, 196)
TemLvzJUKXr.ClipsDescendants = true


local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 9)
UICorner.Parent = TemLvzJUKXr

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = TemLvzJUKXr
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 2.0
UIStroke.Transparency = 0.5

local title = Instance.new("TextLabel")
title.Parent = TemLvzJUKXr
...
-- Smooth Dragging
local d, i, s, p
TemLvzJUKXr.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = TemLvzJUKXr.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
TemLvzJUKXr.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        TemLvzJUKXr.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(TemLvzJUKXr, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -155, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
