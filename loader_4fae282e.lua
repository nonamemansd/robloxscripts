...
local CONFIG = {
    ...
    BG = Color3.fromRGB(27, 26, 21)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_zgRMIvRAWfHc") then CoreGui["KeySystem_zgRMIvRAWfHc"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_zgRMIvRAWfHc"
sg.Parent = CoreGui

local MsFYuYUBIH = Instance.new("Frame")
MsFYuYUBIH.Name = "xUiNJUK"
MsFYuYUBIH.Parent = sg
MsFYuYUBIH.BackgroundColor3 = CONFIG.BG
MsFYuYUBIH.Position = UDim2.new(0.5, -155, 0.5, -96)
MsFYuYUBIH.Size = UDim2.new(0, 311, 0, 192)
MsFYuYUBIH.ClipsDescendants = true


local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 13)
UICorner.Parent = MsFYuYUBIH

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = MsFYuYUBIH
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 1.3
UIStroke.Transparency = 0.7

local title = Instance.new("TextLabel")
title.Parent = MsFYuYUBIH
...
-- Smooth Dragging
local d, i, s, p
MsFYuYUBIH.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = MsFYuYUBIH.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
MsFYuYUBIH.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        MsFYuYUBIH.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(MsFYuYUBIH, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -155, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
