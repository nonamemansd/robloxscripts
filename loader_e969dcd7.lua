...
local CONFIG = {
    ...
    BG = Color3.fromRGB(26, 15, 20)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_GVwgxgRl") then CoreGui["KeySystem_GVwgxgRl"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_GVwgxgRl"
sg.Parent = CoreGui

local eZUEniWku = Instance.new("Frame")
eZUEniWku.Name = "VAMgZUa"
eZUEniWku.Parent = sg
eZUEniWku.BackgroundColor3 = CONFIG.BG
eZUEniWku.Position = UDim2.new(0.5, -152, 0.5, -96)
eZUEniWku.Size = UDim2.new(0, 304, 0, 193)
eZUEniWku.ClipsDescendants = true


local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 8)
UICorner.Parent = eZUEniWku

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = eZUEniWku
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 2.2
UIStroke.Transparency = 0.4

local title = Instance.new("TextLabel")
title.Parent = eZUEniWku
...
-- Smooth Dragging
local d, i, s, p
eZUEniWku.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = eZUEniWku.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
eZUEniWku.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        eZUEniWku.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(eZUEniWku, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -152, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
