...
local CONFIG = {
    ...
    BG = Color3.fromRGB(15, 27, 25)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_LpKkUrzZBG") then CoreGui["KeySystem_LpKkUrzZBG"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_LpKkUrzZBG"
sg.Parent = CoreGui

local QvMHYzDQSwT = Instance.new("Frame")
QvMHYzDQSwT.Name = "PrXQdIJO"
QvMHYzDQSwT.Parent = sg
QvMHYzDQSwT.BackgroundColor3 = CONFIG.BG
QvMHYzDQSwT.Position = UDim2.new(0.5, -145, 0.5, -85)
QvMHYzDQSwT.Size = UDim2.new(0, 291, 0, 170)
QvMHYzDQSwT.ClipsDescendants = true


local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 8)
UICorner.Parent = QvMHYzDQSwT

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = QvMHYzDQSwT
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 2.0
UIStroke.Transparency = 0.6

local title = Instance.new("TextLabel")
title.Parent = QvMHYzDQSwT
...
-- Smooth Dragging
local d, i, s, p
QvMHYzDQSwT.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = QvMHYzDQSwT.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
QvMHYzDQSwT.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        QvMHYzDQSwT.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(QvMHYzDQSwT, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -145, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
