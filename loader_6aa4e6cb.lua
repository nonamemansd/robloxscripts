...
local CONFIG = {
    ...
    BG = Color3.fromRGB(29, 27, 29)
}

local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")

if CoreGui:FindFirstChild("KeySystem_Dpbyqbz") then CoreGui["KeySystem_Dpbyqbz"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_Dpbyqbz"
sg.Parent = CoreGui

local skKWJqWf = Instance.new("Frame")
skKWJqWf.Name = "HAOyyBBa"
skKWJqWf.Parent = sg
skKWJqWf.BackgroundColor3 = CONFIG.BG
skKWJqWf.Position = UDim2.new(0.5, -155, 0.5, -91)
skKWJqWf.Size = UDim2.new(0, 310, 0, 182)
skKWJqWf.ClipsDescendants = true

    local grad = Instance.new("UIGradient")
    grad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, skKWJqWf.BackgroundColor3),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(38, 37, 55))
    })
    grad.Rotation = 0
    grad.Parent = skKWJqWf
    

local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 7)
UICorner.Parent = skKWJqWf

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = skKWJqWf
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 2.0
UIStroke.Transparency = 0.4

local title = Instance.new("TextLabel")
title.Parent = skKWJqWf
...
-- Smooth Dragging
local d, i, s, p
skKWJqWf.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = skKWJqWf.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
skKWJqWf.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        skKWJqWf.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(skKWJqWf, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -155, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
