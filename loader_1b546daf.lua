...
if CoreGui:FindFirstChild("KeySystem_uPblTmgMTKkg") then CoreGui["KeySystem_uPblTmgMTKkg"]:Destroy() end

local sg = Instance.new("ScreenGui")
sg.Name = "KeySystem_uPblTmgMTKkg"
sg.Parent = CoreGui

local GnYzxVSHsjFy = Instance.new("Frame")
GnYzxVSHsjFy.Name = "bzddLleDcG"
GnYzxVSHsjFy.Parent = sg
GnYzxVSHsjFy.BackgroundColor3 = CONFIG.BG
GnYzxVSHsjFy.Position = UDim2.new(0.5, -148, 0.5, -88)
GnYzxVSHsjFy.Size = UDim2.new(0, 296, 0, 177)
GnYzxVSHsjFy.ClipsDescendants = true


local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 9)
UICorner.Parent = GnYzxVSHsjFy

local UIStroke = Instance.new("UIStroke")
UIStroke.Parent = GnYzxVSHsjFy
UIStroke.Color = CONFIG.Accent
UIStroke.Thickness = 1.6
UIStroke.Transparency = 0.6

local title = Instance.new("TextLabel")
title.Parent = GnYzxVSHsjFy
...
-- Smooth Dragging
local d, i, s, p
GnYzxVSHsjFy.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        d = true; s = input.Position; p = GnYzxVSHsjFy.Position
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then d = false end end)
    end
end)
GnYzxVSHsjFy.InputChanged:Connect(function(input)
    if d and input.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = input.Position - s
        GnYzxVSHsjFy.Position = UDim2.new(p.X.Scale, p.X.Offset + delta.X, p.Y.Scale, p.Y.Offset + delta.Y)
    end
end)
...
c_btn.MouseButton1Click:Connect(function()
    ...
    if success and k == actual:gsub("%s+", "") then
        c_btn.Text = "SUCCESS"
        TweenService:Create(GnYzxVSHsjFy, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -148, 1.5, 0)}):Play()
        task.wait(0.5)
        sg:Destroy()
